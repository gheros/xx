...
def svndownbase(filename):
    ...
    print(svnco+svnserver+'/'+filename+' '+svnlocal+'/'+filename)
    print(a.read())
...
def up(string,string2):
    svnupbase('xjdeny',string,string2)
    writeup()
    svnupbase('up',string,string2)
    svnrequest()


# svn.wbapi.com/tool.php?opt=nginx-web-conf
# down()
# up('test','test1')
...
#     xjdenypath = config.get("svn", "xjdenypath")
#     uppath = config.get("svn", "uppath")
#     b=os.popen("svn st "+xjdenypath+"|awk '{print $2}'|xargs svn add")
#     b1=os.popen("svn st "+uppath+"|awk '{print $2}'|xargs svn add")
#     c=os.popen("/usr/bin/svn ci -m '"+"程序上传xjdeny操作"+"' "+xjdenypath+" 2>&1")
#     c1=os.popen("/usr/bin/svn ci -m '"+"程序上传UP操作"+"' "+uppath+" 2>&1")
#     print(b.read())
